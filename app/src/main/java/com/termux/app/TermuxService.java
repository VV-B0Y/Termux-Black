...
import com.termux.app.usb.QmpClient;
import com.termux.app.usb.UsbPassthroughManager;
import com.termux.app.usb.UsbShimInstaller;
...
public final class TermuxService extends Service implements AppShell.AppShellClient, TermuxSession.TermuxSessionClient {
    ...
    public static final String ACTION_ATTACH_USB = "com.termux.service_attach_usb";
    ...
    private UsbPassthroughManager mUsbPassthroughManager;
    ...
    public void onCreate() {
        ...
        // QMP socket of the trixie_vm QEMU instance, used for USB passthrough.
        String qmpSockPath = com.termux.app.rootless.RootlessVm.qmpSocketPath(this);
        mUsbPassthroughManager = new UsbPassthroughManager(this, new QmpClient(qmpSockPath));

        // Deploy the LD_PRELOAD shim and make `vm` preload it into qemu. Must happen before the
        // VM starts, otherwise QEMU's libusb_init fails and device_add is rejected.
        UsbShimInstaller.install(this);
        ...
        if (action != null) {
            switch (action) {
                ...
                case TERMUX_SERVICE.ACTION_SERVICE_EXECUTE:
                    ...
                    break;
                case ACTION_ATTACH_USB:
                    Logger.logDebug(LOG_TAG, "ACTION_ATTACH_USB intent received");
                    actionAttachUsb(intent);
                    break;
                case ACTION_DETACH_USB:
                    Logger.logDebug(LOG_TAG, "ACTION_DETACH_USB intent received");
                    actionDetachUsb();
                    break;
                default:
    ...
    private void actionAttachUsb(Intent intent) {
        if (mUsbPassthroughManager == null) return;
        new Thread(() -> {
            UsbDevice device = intent != null ? (UsbDevice) intent.getParcelableExtra(UsbManager.EXTRA_DEVICE) : null;
            if (device != null) {
                mUsbPassthroughManager.attachAsync(device, (ok, d) -> {
                    Logger.logInfo(LOG_TAG, "USB attach result for " + (d != null ? d.getDeviceName() : "device") + ": " + ok);
                });
            } else {
                int count = mUsbPassthroughManager.attachAllWifiDongles(5000);
                Logger.logInfo(LOG_TAG, "Attached " + count + " USB Wi-Fi dongles to VM");
            }
        }).start();
    }

    private void actionDetachUsb() {
        if (mUsbPassthroughManager != null) {
            mUsbPassthroughManager.detachAll();
        }
    }
